...
export function InviteDriverModal({ order, canInviteById, canAcceptInvite }: InviteDriverModalProps) {
	...
	const inviteToken = inviteData?.invite_token ?? acceptToken

	const shareLink = useMemo(() => {
		if (!inviteToken) return ''
		if (typeof window === 'undefined') return ''
		return `${window.location.origin}${DASHBOARD_URL.order(`invite/${inviteToken}`)}`
	}, [inviteToken])
	...
	const handleGenerateInviteLink = () => {
		...
		generateOrderInvite(
			...
			{
				onSuccess: (data) => {
					const token = (data as OrderInviteResult)?.invite_token
					if (token) {
						setAcceptToken((prev) => prev || token)
					}
				},
			},
		)
		...
		<Dialog open={isOpen} onOpenChange={handleModalOpenChange}>
			<DialogTrigger asChild>
				<Button className="bg-brand text-white">
					{canInviteById ? 'Пригласить водителя' : 'Принять приглашение'}
				...
				<Card className="border-none shadow-none">
					<CardContent className="flex flex-col gap-6 pt-6">
						...
						<div className="flex flex-col gap-4">
							...
							{canInviteById && (
								<div className="space-y-2">
									...
									<p className="text-sm text-muted-foreground">
										Сгенерируйте ссылку и отправьте её водителю, чтобы он смог принять приглашение.
									</p>
									<InputGroup className="bg-background">
										<InputGroupInput
											readOnly
									...
									{shareCopyStatus === 'copied' && (
										<p className="text-sm text-success-500">Ссылка скопирована.</p>
									)}
							...
							{canAcceptInvite && (
								<div className="space-y-2">
									<p className="text-sm font-semibold text-foreground">Принять приглашение</p>
									<p className="text-sm text-muted-foreground">
										Введите токен приглашения, чтобы стать водителем в этом заказе.
									</p>
									<InputGroup className="bg-background">
										<InputGroupInput
											value={acceptToken}
										...
										<InputGroupAddon align="inline-end">
											...
											>
												{isLoadingAcceptInvite ? 'Обработка...' : 'Принять приглашение'}
												<Link2 className="size-4" />
											</Button>
										</InputGroupAddon>
									</InputGroup>
						...
						<div className="mt-2 flex max-md:flex-col md:justify-end gap-3">
							<DialogClose asChild>
								<Button className="max-md:w-full bg-destructive text-white hover:bg-destructive/90" type="button">
									Отменить
								</Button>
							</DialogClose>
						</div>
